...
- **Docs infra — plots for the GP-UDE examples (deferred).** The GP-UDE example pages render as **static** `julia` blocks in `docs/make.jl` (`gp_ude_*` → `codefence = "```julia" => "```"`), *not* executed `@example` blocks, because each is a through-solver training (~4-8 min) and executing all of them in the docs build cost ~35-99 min (scale-forcing alone ~78 min, now dropped from docs). The trade: the rendered GP-UDE pages show narrative + code but **no plots**. To restore plots without re-slowing the build, **decouple plot generation from the docs build**: the `docs-examples` anti-rot jobs already *execute* each example (and its `savefig` calls), so publish those `.png`s as CI artifacts (or commit them) and reference them from the example `.jl` via `# ![](…)` lines Literate carries into the static page. (Re-executing in docs — flipping back to `@example` — would re-introduce the slow build.) Secondary tweak: the examples `savefig` then end the block, so even under `@example` the plot object isn't displayed inline — embedding needs a small per-example change regardless.
