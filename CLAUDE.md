...
**Spikes resolved** (June 2026, `docs/research/spike-results.md`): (1) GP-in-ODE through-solver diff ✅ works (`GaussAdjoint`+`ReverseDiffVJP`, outer Mooncake; `MooncakeVJP` exists but unexported, `MooncakeAdjoint` buggy); (3) `update_chol` under Mooncake ✅ works (so reuse it). (2) Mooncake through Laplace ❌ fails (a `@debug`-macro `try/catch`, not the Newton numerics — likely a cheap fix; else the implicit-diff Laplace adjoint), gating binary BALD. Remaining empirical work: GP-UDE cost at scale; a Laplace-fix spike; a MooncakeVJP-vs-ReverseDiffVJP benchmark.
