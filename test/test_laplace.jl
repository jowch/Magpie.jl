using AlphaGP, AbstractGPs, KernelFunctions, LinearAlgebra, DifferentiationInterface, Test
using AlphaGP: LaplaceGP
import Mooncake
@testset "LaplaceGP" begin
    ...
    g = AlphaGP.update(LaplaceGP(with_lengthscale(SqExponentialKernel(),1.0)), X, yb)
    ...
    @test mean(g, [[0.0]]) ≈ [dot(AbstractGPs.cov(g.prior, g.x, [[0.0]]), g.a)] rtol=1e-8
    loss(logℓ) = sum(mean(AlphaGP.update(LaplaceGP(with_lengthscale(SqExponentialKernel(),exp(only(logℓ)))), X, yb), X))
    gmc = only(DifferentiationInterface.gradient(loss, AutoMooncake(;config=nothing), [0.0]))
    gfd = (loss([1e-5]) - loss([-1e-5])) / 2e-5
    @test gmc ≈ gfd rtol=1e-3
end
