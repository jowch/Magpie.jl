...
@testset "incremental == batch" begin
    ...
    for i in 1:8; g_inc = Magpie.update(g_inc, X[i], y[i]); end
...
@testset "mean_and_var / predmean / predict" begin
    ...
    g = Magpie.update(ExactGP(k; noise=1e-3), [randn(2) for _ in 1:6], randn(6))
    u = randn(2)
    μ, v = mean_and_var(g, [u])
    @test μ ≈ mean(g, [u]) && v ≈ var(g, [u])
    @test Magpie.predmean(g, u) ≈ mean(g, [u])[1] rtol=1e-12
