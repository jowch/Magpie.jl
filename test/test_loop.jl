...
using Magpie: ExactGP, Straddle, ActiveLearner, observe!, acquire, posterior_gp, all_data, Box
...
@testset "ActiveLearner loop == batch" begin
    ...
    g_batch = Magpie.update(ExactGP(with_lengthscale(SqExponentialKernel(),0.5); noise=1e-4), X, f.(X))
