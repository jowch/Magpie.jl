...
@testset "Straddle" begin
    g = Magpie.update(ExactGP(with_lengthscale(SqExponentialKernel(),0.5); noise=1e-4), [[0.0],[1.0]], [0.0,1.0])
    ...
    g0 = Magpie.update(ExactGP(with_lengthscale(SqExponentialKernel(),0.5); noise=1e-10), [[0.0]], [0.0])
...
@testset "RandStraddle + resample" begin
    ...
    g = Magpie.update(ExactGP(with_lengthscale(SqExponentialKernel(),0.5); noise=1e-3), [[0.0],[1.0]], [0.0,1.0])
